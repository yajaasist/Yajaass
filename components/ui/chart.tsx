      ...
      dangerouslySetInnerHTML={{
        __html: Object.entries(colorConfig)
          .map(
            ([key, itemConfig]) => `
[data-chart=${id}] {
  --color-${key}: ${itemConfig[1]?.color ?? ""};
}
.dark [data-chart=${id}] {
  --color-${key}: ${
              itemConfig[1]?.theme?.dark ?? itemConfig[1]?.color ?? ""
            };
}
`
          )
...
function getPayloadConfigFromPayload(
  ...
  if (
    ...
    const nameKeyValue = payloadPayload[key] as string
    if (nameKeyValue && nameKeyValue in config) {
      configLabelKey = nameKeyValue
    }
  }
